...
namespace FireFishPortfolioManager.Functions
{
    public class BitcoinPriceUpdater
    {
        ...
        private async Task<decimal> GetCoinmatePriceAsync(string currencyPair)
        {
            var url = $"https://coinmate.io/api/ticker?currencyPair={currencyPair}";
            ...
            throw new Exception($"Invalid response from Coinmate API for {currencyPair}: 'last' price not found.");
        }
    }
} 