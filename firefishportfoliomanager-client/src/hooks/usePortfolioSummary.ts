...
export const usePortfolioSummary = (): PortfolioSummary | null => {
  ...
  const portfolioSummary = useMemo((): PortfolioSummary | null => {
    ...
    const totalLoanAmountCzk = activeLoans.reduce((sum, loan) => sum + loan.loanAmountCzk, 0);
    ...
    const totalInvestedCzk = loans.reduce((sum, loan) => sum + loan.loanAmountCzk, 0); // Sum of all initial loan amounts
    const estimatedProfit = currentTotalBtcValue - totalInvestedCzk; // Very basic estimation

    return {
      // User specific data
      allocatedBtc: user.allocatedBtc ?? 0,
      //targetLtv: user.targetLtv, // odstraněno
      //maxLoanAmount: user.maxLoanAmount, // odstraněno
